...
class CVAE_dropout(nn.Module):
    def __init__(self, img_channels=3, z_dim=256, img_size=64, dropout_prob=0.5):
        ...
        # Determine the size of thz_dime output of the last convolutional layer
    ...
    def encoder(self, x):
        ...
        x = self.dropout4(x)
        # flattening the output of droput4() layer
        x = x.view(
            -1,         # here -1 argument indicates that the size of that dimension should be inferred based on the size of the other dimensions and the total number of elements in the tensor
            256
    ...
    def decoder(self, z):
        ...
        # flattening output
        x = x.view(
            -1,         # The -1 argument indicates that the size of the first dimension (batch size) should be inferred, similar to the encoder's flattening operation
            256,
            self.calculate_conv_output_size(self.img_size),
            self.calculate_conv_output_size(self.img_size),
        )  # Reshape
